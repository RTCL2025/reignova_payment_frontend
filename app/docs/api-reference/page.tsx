...
export const metadata = {
  ...
  description: 'Complete interactive REST API endpoint specifications and schema definitions.',
};
  ...
  return (
    ...
    >
      <div className="space-y-8">
        {endpoints.map((ep) => (
          <div key={ep.path} id={ep.path} className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-3">
                <Badge className={ep.method === 'POST' ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-sky-100 text-sky-800 border-sky-300'}>
                  {ep.method}
                </Badge>
                <h3 className="font-bold text-slate-900 text-base">{ep.title}</h3>
              </div>
              <code className="text-xs font-mono text-amber-700 font-semibold">{ep.path}</code>
            </div>
            ...
            {ep.headers.length > 0 && (
              <div className="space-y-1">
                <span className="text-[11px] font-mono text-slate-500 font-bold uppercase">Required Headers</span>
                <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200 font-mono text-xs text-slate-800">
                  {ep.headers.map(h => <div key={h}>{h}</div>)}
                </div>
              </div>
            )}
            ...
            {ep.body && (
              <div className="space-y-1">
                <span className="text-[11px] font-mono text-slate-500 font-bold uppercase">Request Body</span>
            ...
            <div className="space-y-1">
              <span className="text-[11px] font-mono text-slate-500 font-bold uppercase">Response (200 OK)</span>
