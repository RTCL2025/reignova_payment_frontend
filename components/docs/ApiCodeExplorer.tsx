...
import { Copy, Check, Terminal, Code, Layers, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export const API_ENDPOINTS: ApiEndpointExample[] = [
  {
    ...
    description: 'Generates a hosted payment session public token for customer checkout.',
    curl: `curl -X POST https://pay.reignovatechnologies.com/api/v1/checkouts/public \\
  ...
  -d '{
    ...
    js: `const response = await fetch('https://pay.reignovatechnologies.com/api/v1/checkouts/public', {
  method: 'POST',
  headers: {
...
const res = await fetch('https://pay.reignovatechnologies.com/api/v1/checkouts/public', {
  method: 'POST',
  headers: {
...
url = "https://pay.reignovatechnologies.com/api/v1/checkouts/public"
headers = {
    "Authorization": "Bearer sk_live_app_8f3a",
    "Content-Type": "application/json"
}
...
print(response.json())`,
    php: `<?php
$ch = curl_init('https://pay.reignovatechnologies.com/api/v1/checkouts/public');
...
curl_close($ch);`,
  ...
  },
  {
    ...
    description: 'Triggers a mobile money push payment prompt for a specific checkout token.',
    curl: `curl -X POST https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/pay \\
  -H "Content-Type: application/json" \\
  -d '{
    "provider": "VODACOM_TZ",
    "customerPhone": "+255712345678"
  }'`,
    js: `const res = await fetch('https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/pay', {
  method: 'POST',
...
const result = await res.json();`,
  ...
  customerPhone: '+255712345678'
    ...
    "https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/pay",
...
$payload = json_encode(['provider' => 'VODACOM_TZ', 'customerPhone' => '+255712345678']);`,
  ...
  },
  {
    ...
    description: 'Fetches the current status of a checkout session and underlying deposit attempt.',
    curl: `curl -X GET https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/status`,
    js: `const res = await fetch('https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/status');
const status = await res.json();`,
    ts: `const statusData = await getCheckoutStatus("chk_pub_98a7b6c51120");`,
    python: `res = requests.get("https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/status")`,
    php: `$res = file_get_contents("https://pay.reignovatechnologies.com/api/v1/checkouts/public/chk_pub_98a7b6c51120/status");`,
    successResponse: `{
  "success": true,
  "data": {
    "status": "COMPLETED",
...
export function ApiCodeExplorer() {
  ...
  const [activeLang, setActiveLang] = useState<'curl' | 'js' | 'ts' | 'python' | 'php'>('curl');
  ...
  const getCodeForLang = () => {
    switch (activeLang) {
      ...
      case 'js': return currentEndpoint.js;
      case 'ts': return currentEndpoint.ts;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-12">
          ...
          <p className="text-sm sm:text-base text-slate-600 leading-relaxed">
            Inspect request payloads, HTTP headers, multi-language SDK snippets, and structured error responses.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {API_ENDPOINTS.map((ep, idx) => (
            ...
            >
              <Badge className={`text-[10px] ${getMethodBadgeClass(ep.method)}`}>{ep.method}</Badge>
        ...
        <div className="bg-[#0A121A] border border-slate-800 rounded-2xl overflow-hidden shadow-xl grid grid-cols-1 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7 border-b lg:border-b-0 lg:border-r border-slate-800 flex flex-col justify-between">
            ...
            <div className="px-4 py-3 bg-[#131E2A] border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Badge className={`text-[11px] font-mono ${getMethodBadgeClass(currentEndpoint.method)}`}>
                  {currentEndpoint.method}
                </Badge>
                <span className="text-xs font-mono text-slate-200">{currentEndpoint.endpoint}</span>
              </div>
                ...
                {(['curl', 'js', 'ts', 'python', 'php'] as const).map((lang) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded capitalize transition-colors ${
                      activeLang === lang ? 'bg-[#F3A221] text-slate-950 font-bold' : 'text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    {lang === 'curl' ? 'cURL' : lang}
                  </button>
            ...
            <div className="p-4 font-mono text-xs text-amber-300 overflow-x-auto bg-[#0A121A] min-h-[220px]">
              <div className="text-slate-500 mb-2">// {currentEndpoint.title} — {currentEndpoint.description}</div>
              <pre><code>{getCodeForLang()}</code></pre>
            </div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between bg-[#0F1A25]/60">
            ...
            {/* Header bar */}
            <div className="px-4 py-3 bg-[#131E2A] border-b border-slate-800 flex items-center justify-between">
              <span className="text-xs font-mono text-slate-400 font-bold">RESPONSE BODY</span>
              <div className="flex bg-slate-900 p-0.5 rounded-lg border border-slate-800 text-[11px] font-mono">
                <button
                  onClick={() => setViewError(false)}
                  className={`px-2.5 py-1 rounded transition-colors ${
                    !viewError ? 'bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/40' : 'text-slate-400'
                  }`}
                >
                  200 OK
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded transition-colors ${
                    viewError ? 'bg-rose-500/20 text-rose-300 font-bold border border-rose-500/40' : 'text-slate-400'
                  }`}
                >
                  Error Response
                </button>
            ...
            <div className="p-4 font-mono text-xs overflow-x-auto min-h-[220px]">
              <pre className={viewError ? 'text-rose-300' : 'text-emerald-300'}>
                <code>{viewError ? currentEndpoint.errorResponse : currentEndpoint.successResponse}</code>
              </pre>
            </div>
            ...
            <div className="px-4 py-2.5 bg-[#131E2A] border-t border-slate-800 text-[11px] font-mono text-slate-400 flex items-center justify-between">
              <span className="flex items-center gap-1">
                {viewError ? <AlertTriangle className="size-3.5 text-rose-400" /> : <Check className="size-3.5 text-emerald-400" />}
                <span>{viewError ? 'Structured RFC-7807 Error Payload' : 'Standard JSON Response'}</span>
              </span>
